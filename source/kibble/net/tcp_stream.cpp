...
ssize_t TCPStream::send(const char* buffer, size_t len) { return write(fd_, buffer, len); }

ssize_t TCPStream::receive(char* buffer, size_t len) { return read(fd_, buffer, len); }
