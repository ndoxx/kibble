...
template <typename NodeT> void flatten(const NodeT& node, const std::string& name_chain, SettingsStorage& storage)
{
    ...
    {
        storage.scalars.insert({name_hash, SettingsScalar{*node}});
    }
...
template <typename NodeT> void serialize(NodeT& node, const std::string& name_chain, SettingsStorage& storage)
{
    hash_t name_hash = H_(name_chain.c_str());
...
hash_t Settings::get_hash(hash_t hash, const std::string& def) { return H_(get(hash, def)); }
