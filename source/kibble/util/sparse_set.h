...
public:
    ...
    void insert(const T& val)
    {
        if (!has(val))
        {
            K_ASSERT(val < SIZE, "Exceeding SparseSet capacity: {} / {}", val, SIZE);
...
public:
    ...
    T acquire()
    {
        K_ASSERT(size_ + 1 < SIZE, "Exceeding SparsePool capacity: {} / {}", size_, SIZE);
    ...
    {
        K_ASSERT(is_valid(handle), "Cannot release unknown handle: {}", handle);
...
public:
    ...
    T acquire()
    {
        K_ASSERT(size_ + 1 < SIZE, "Exceeding SecureSparsePool capacity: {} / {}", size_, SIZE);
    ...
    void release(T handle)
    {
        K_ASSERT(is_valid(handle), "Cannot release unknown handle: [{}|{}]", size_t(unguard(handle)),
