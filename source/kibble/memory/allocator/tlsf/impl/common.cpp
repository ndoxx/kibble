...
void mapping_insert(size_t size, int& fli, int& sli)
{
    ...
    {
        fli = fls_size_t(size);
        sli = (int(size) >> (fli - int(k_sl_index_count_log2))) ^ int(1ull << k_sl_index_count_log2);
        fli -= int(k_fl_index_shift - 1);
    }
}
...
void mapping_search(size_t size, int& fli, int& sli)
{
    if (size >= k_small_block_size)
    {
        const size_t round = (1ull << (size_t(fls_size_t(size)) - k_sl_index_count_log2)) - 1ull;
        size += round;
    }
