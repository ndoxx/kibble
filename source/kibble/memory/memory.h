...
template <typename SynchronizationPrimitiveT> class MultiThread
{
public:
...
public:
    ...
    ~MemoryArena() { memory_tracker_.report(); }

    template <typename... ArgsT> inline void init(ArgsT&&... args)
    {
        allocator_.init(std::forward<ArgsT>(args)...);
        is_initialized_ = true;
    }

    inline void shutdown() { is_initialized_ = false; }
...
template <typename T, class ArenaT> void Delete(T* object, ArenaT& arena)
{
    ...
    // BUG/HACK:
    // Sometimes, the address returned by K_NEW is not the same as the one passed to the underlying
...
template <class T> struct TypeAndCount
{
};

template <class T, size_t N> struct TypeAndCount<T[N]>
{
    typedef T type;
    static constexpr size_t count = N;
};

template <typename ThreadPolicyT = policy::SingleThread> class LinearBuffer
{
public:
    LinearBuffer() = default;
    LinearBuffer(kb::memory::HeapArea& area, std::size_t size, const char* debug_name) { init(area, size, debug_name); }
    ...
    template <typename T> inline void write(T* source) { write(source, sizeof(T)); }
    template <typename T> inline void read(T* destination) { read(destination, sizeof(T)); }
