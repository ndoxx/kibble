...
int main(int argc, char **argv)
{
    ...
    parser.set_log_output([&chan_istr](const std::string &str) { klog2(chan_istr).uid("ArgParse").info(str); });
    ...
    {
        klog2(chan_istr).fatal("Directory does not exist:\n{}", dirpath);
    }
    if (!fs::is_directory(dirpath))
    {
        klog2(chan_istr).fatal("Not a directory:\n{}", dirpath);
    }
    ...
    {
        klog2(chan_istr).fatal("Output directory does not exist:\n{}", outputdir);
    }

    fs::path outputpath = (outputdir / "intern.txt").lexically_normal();

    // * Locate sources
    klog2(chan_istr).info("Parsing sources.");
    klog2(chan_istr).info("root: {}", dirpath);
    ...
    {
        klog2(chan_istr).info("Detected manifest.");
    ...
    {
        klog2(chan_istr).fatal("Unable to open output text file.");
    }
    klog2(chan_istr).info("Done.");
                            ...
                            const kb::log::Channel &log_channel)
{
    ...
    if (it == registry.end())
    {
        klog2(log_channel).verbose("{:<20} -> {}", hash_intern, intern);
                 ...
                 kfs::FileSystem &filesystem, const kb::log::Channel &log_channel)
{
    ...
    klog2(log_channel).info("reading {}", fs::relative(entry.path(), base));
